...
export default function TriageForm() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-2 animate-in fade-in slide-in-from-bottom-4 duration-500">
      
      {result && (() => {
        const colorCode = getColorCode(result.esi_level);
        return (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className={`w-full max-w-lg p-6 rounded-3xl shadow-2xl border-4 ${getColorClass(colorCode)} transform transition-all scale-100 animate-in zoom-in-95 duration-300`}>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-3xl font-black tracking-tight">Niveau ESI : {result.esi_level}</h3>
                <span className="text-2xl font-black uppercase tracking-widest opacity-90">{colorCode}</span>
              </div>
              <p className="text-lg font-medium opacity-95 leading-relaxed">{result.esi_explanation}</p>
              <div className="mt-6 space-y-2">
                <button
                  onClick={() => generateTiragePDF({ ...result, color_code: colorCode }, form, operatorFullName)}
                  className="w-full bg-white/90 hover:bg-white text-slate-800 font-bold py-2.5 px-6 rounded-xl transition-colors border border-white/50 text-sm shadow-sm flex items-center justify-center gap-2"
                >
                ...
                <button 
                  ...
                  className="w-full bg-white/20 hover:bg-white/30 text-current font-bold py-2.5 px-6 rounded-xl backdrop-blur-sm transition-colors border border-white/30 text-sm shadow-sm flex items-center justify-center gap-2"
                >
      ...
      <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-slate-800 flex items-center">
            ...
            Évaluation Médicale ESI
          </h2>
          <div className="text-xs font-bold text-slate-400 uppercase bg-slate-100 px-3 py-1 rounded-full border border-slate-200">
            Agent : {operatorFullName}
          </div>
        </div>
        
        {error && <div className="bg-red-50 text-red-700 p-2.5 rounded-lg mb-4 border border-red-200 text-xs">{error}</div>}
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-5">
            ...
            <div className="p-4 bg-slate-50/50 border border-slate-100 rounded-xl">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 border-b border-slate-200 pb-1">Appelant (Optionnel)</h3>
              <div className="grid grid-cols-4 gap-3">
                <div className="col-span-2">
                  <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Nom / Prénom</label>
                  <div className="flex gap-2">
                    <input type="text" name="caller_name" value={form.caller_name} onChange={handleChange}
                      className="w-1/2 bg-white border border-slate-200 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500 p-2 text-sm" placeholder="Nom" />
                    <input type="text" name="caller_surname" value={form.caller_surname} onChange={handleChange}
                      className="w-1/2 bg-white border border-slate-200 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500 p-2 text-sm" placeholder="Prénom" />
                  </div>
                </div>
                <div>
                  ...
                  <input type="number" name="caller_age" value={form.caller_age} onChange={handleChange} min="0" max="120"
                    className="w-full bg-white border border-slate-200 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500 p-2 text-sm" placeholder="Ans" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Sexe</label>
                  <select name="caller_sex" value={form.caller_sex} onChange={handleChange} className="w-full bg-white border border-slate-200 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500 p-2 text-sm">
                    <option value="">...</option>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1 tracking-tight">ID Patient / Lieu *</label>
                  <input type="text" name="patient_identifier" value={form.patient_identifier} onChange={handleChange} required
                    className="w-full bg-slate-50 border border-slate-300 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500 p-2.5 text-sm" placeholder="Nom ou Lieu..." />
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1 tracking-tight">Âge Patient *</label>
                  <input type="number" name="patient_age" value={form.patient_age} onChange={handleChange} required min="0" max="120"
                    className="w-full bg-slate-50 border border-slate-300 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500 p-2.5 text-sm" placeholder="Ans" />
                </div>
              </div>
              
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1 tracking-tight">N° SMUR / Ambulance</label>
                <input type="text" name="smur_number" value={form.smur_number} onChange={handleChange}
                  className="w-full bg-slate-50 border border-slate-300 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500 p-2.5 text-sm" placeholder="Ex: 476" />
              </div>
              
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1 tracking-tight">Motif de l'appel *</label>
                <textarea name="symptoms_description" value={form.symptoms_description} onChange={handleChange} required rows="3"
                  className="w-full bg-slate-50 border border-slate-300 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500 p-2.5 text-sm leading-relaxed" placeholder="Résumé des plaintes..."></textarea>
              </div>
            </div>
          ...
          <div className="space-y-5">
            ...
            <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100 space-y-4 shadow-xs">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-black text-blue-800 uppercase tracking-tight mb-1">Domaine Médical</label>
                  <select name="medical_category" value={form.medical_category} onChange={handleChange} className="w-full bg-white border border-slate-300 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500 p-2 text-sm font-bold">
                    <option value="Médecine Générale">Médecine Générale</option>
                    <option value="Chirurgie">Chirurgie</option>
                    <option value="Gynécologie - Obstétrique">Gynéco-Obs</option>
                    <option value="Pédiatrie">Pédiatrie</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-black text-blue-800 uppercase tracking-tight mb-1">Filtrer Symptômes</label>
                  <input 
                    type="text" 
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    placeholder="Rechercher..."
                    className="w-full bg-white border border-slate-300 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500 p-2 text-sm"
                  />
                </div>
              </div>
            ...
            <div className={`transition-all duration-300 ${form.specific_symptom !== 'Aucun' && form.specific_symptom !== '' ? 'opacity-30 grayscale pointer-events-none scale-98' : 'opacity-100'}`}>
              <div className="grid grid-cols-4 gap-3">
                <div className="col-span-2 bg-slate-50 p-2 rounded-lg border border-slate-200">
                  <label className="block text-[10px] font-black text-slate-500 mb-1 uppercase">Catégorie Âge</label>
                  <select name="age_category" value={form.age_category} onChange={handleChange} className="w-full bg-white border border-slate-300 text-slate-900 rounded-md p-1.5 text-xs font-bold">
                ...
                <div className="bg-slate-50 p-2 rounded-lg border border-slate-200">
                  <label className="block text-[10px] font-black text-slate-500 mb-1 uppercase">Pouls</label>
                ...
                <div className="bg-slate-50 p-2 rounded-lg border border-slate-200">
                  <label className="block text-[10px] font-black text-slate-500 mb-1 uppercase">Resp</label>
                  <input type="number" name="fr" value={form.fr} onChange={handleChange} placeholder="m/min" className="w-full bg-white border border-slate-300 text-slate-900 rounded-md p-1.5 text-xs font-bold" />
                </div>
              </div>

              <div className="grid grid-cols-4 gap-3 mt-3">
                <div className="bg-slate-50 p-2 rounded-lg border border-slate-200">
                  <label className="block text-[10px] font-black text-slate-500 mb-1 uppercase">PAS</label>
                ...
                <div className="bg-slate-50 p-2 rounded-lg border border-slate-200">
                  <label className="block text-[10px] font-black text-slate-500 mb-1 uppercase">GCS</label>
                  <input type="number" name="gcs" value={form.gcs} onChange={handleChange} min="3" max="15" className="w-full bg-white border border-slate-300 text-slate-900 rounded-md p-1.5 text-xs font-bold" />
                </div>
                <div className="bg-slate-50 p-2 rounded-lg border border-slate-200">
                  <label className="block text-[10px] font-black text-slate-500 mb-1 uppercase">Douleur</label>
                  <input type="number" name="pain_scale" value={form.pain_scale} onChange={handleChange} min="0" max="10" className="w-full bg-white border border-slate-300 text-slate-900 rounded-md p-1.5 text-xs font-bold" />
                </div>
              </div>

              <div className="bg-slate-50 p-2 rounded-lg border border-slate-200 mt-3">
                  <label className="block text-[10px] font-black text-slate-500 mb-1 uppercase text-center">Ressources Estimées (0, 1, 2+)</label>
                  <div className="flex gap-2">
                    {[0, 1, 2].map(num => (
                      <button key={num} type="button" onClick={() => setForm(prev => ({...prev, estimated_resources: num}))}
                        className={`flex-1 py-1.5 rounded-lg text-sm font-bold border transition-all ${form.estimated_resources === num ? 'bg-slate-800 text-white border-slate-900 shadow-sm' : 'bg-white text-slate-600 border-slate-200'}`}>
                        {num === 2 ? '2+' : num}
                      </button>
                    ))}
                  </div>
            ...
            <button type="submit" disabled={loading} className="w-full bg-slate-900 hover:bg-black text-white font-black rounded-xl text-md py-4 text-center transition-all shadow-lg hover:shadow-xl disabled:opacity-70 flex justify-center items-center gap-2 uppercase tracking-widest">
